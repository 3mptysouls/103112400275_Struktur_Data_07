...
```text
Sebelum ditukar: a = 10, b = 20
Setelah ditukar: a = 20, b = 10
```
...
int main()
{
    ...
    int matriksDafa[3][3]; // matriks baru gng

    for (int i = 0; i < 3; ++i)
    {
        for (int j = 0; j < 3; ++j)
        {
            matriksDafa[i][j] = matriks[j][i]; // masukin value ke matriks baru pamke logika transpose
        }
    }
...
Pada program, langkah awal yang dilakukan tentunya mendefinisikan matriksnya terlebih dahulu. Untuk **matriks awal** ini berukuran 3x3 yang dinamakan `matriks` diikuti dengan mengisi matriks, lalu dilanjutkan dengan mendefinisikan **matriks akhir** 3x3 yang dinamakan `matriksDafa`.

Langkah selanjutnya yaitu merancang logika untuk menyimpan value sekaligus transpose matrix. Terdapat **loop** ( `i` ) dan **nested loop** ( `j` ), dengan `i` untuk kolom dan `j` untuk baris. Dalam **nested loop**, kita akan simpan dan transpose dengan cara memasukkan value dari variabel `matriks[j][i]` ke dalam `matriksDafa[i][j]` yang jika ditulis dalam code **C++** adalah `matriksDafa[i][j] = matriks[j][i]`.
...
Nilai setelah dikuadratkan: 25
```
Program kuadrat dengan prosedur call by reference. Call by reference bekerja dengan cara setelah prosedur `kuadratkan(dafa);` pada **main** dipanggil maka value dari `dafa` akan langsung berubah sesuai apa yang terjadi di prosedur (dalam kasus ini dikuadratkan). Maka contoh jika value awal `dafa` adalah 5, setelah prosedur dan dikuadratkan maka value original dari `dafa` akan berubah menjadi 25.
